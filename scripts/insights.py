...
# SOURCE ATTRIBUTION
# ─────────────────────────────────────────
_REF_RE = re.compile(r"(fireflies|notion|emails?)\[(\d+)\]")
_KIND_TO_KEY = {"fireflies": "fireflies", "notion": "notion", "email": "emails", "emails": "emails"}
...
# ─────────────────────────────────────────
def fetch_notion_deals(top_n=15, body_for_top=5):
    """Query the deals pool, filter to 3 active statuses, sort by last_edited desc.
...
deal-funnel updates, weekly memos) into {N_INSIGHTS} tweet drafts in EACH of two voices,
engineered to go viral on crypto Twitter.
{feedback}
...
YOU ARE WRITING IN TWO VOICES — produce {N_INSIGHTS} insights for EACH:
...
Notion deal #5, source_refs = ["fireflies[2]", "notion[5]"]. Always include at
least one ref.
...
def sonnet_distill(payload):
    ...
    user_msg = (
        "Last 7 days of anonymized fund-internal data. Use the `idx` field on each "
        "entry to cite sources in your source_refs output.\n\n"
...
def run(dry_run=False, verbose=True):
    ...
    # Fetch — Notion (optional, env-gated)
    ...
    # Distill
    log(f"  [S] Sonnet distill → {N_INSIGHTS} drafts × 2 voices...")
    try:
        parsed, usage, cost = sonnet_distill(clean_payload)
    except Exception as e:
